...
export class GoogleSheetsService {
  ...
  // In-flight deduplication to avoid multiple simultaneous requests to Google Sheets
  private static inFlightFetchPromise: Promise<{
    woRows: any[][],
    poRows: any[][],
    petugasRows: any[][],
    ulpRows: any[][],
  ...
  private static async fetchSheetDataRaw(sheetName: string): Promise<any[][]> {
    ...
    for (const url of endpoints) {
      ...
      try {
        ...
        clearTimeout(timeoutId);
        // Silent error, try next endpoint
      }
    }
  ...
  private static parseSheetDate(dateStr: string): Date | null {
    ...
    // 0. Handle Excel/Google Sheets serial dates (e.g., 46125.6963 or 46125,6963)
    const normalizedSerial = cleanStr.replace(',', '.');
    if (/^\d{5}(\.\d+)?$/.test(normalizedSerial)) {
      const serial = parseFloat(normalizedSerial);
  ...
  static async fetchData(startDate?: string, endDate?: string, selectedUlp?: string, bypassCache = false): Promise<DashboardData> {
    ...
    const canUseRawCache = !bypassCache && this.rawDataCache && (now - this.rawDataCache.timestamp < 30000);
    ...
    if (canUseRawCache) {
      ...
      if (!this.inFlightFetchPromise) {
        this.inFlightFetchPromise = (async () => {
          const [w, p, pet, u, pos, r, c, reg, anom] = await Promise.all([
      ...
      const fetched = await this.inFlightFetchPromise;
      woRows = fetched.woRows;
      poRows = fetched.poRows;
      petugasRows = fetched.petugasRows;
      ulpRows = fetched.ulpRows;
      poskoRows = fetched.poskoRows;
      ratingRows = fetched.ratingRows;
      cctvDataRows = fetched.cctvDataRows || [];
      reguRows = fetched.reguRows || [];
      anomaliRows = fetched.anomaliRows || [];
