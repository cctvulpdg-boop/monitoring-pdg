...
export const classifyAnomaly = (jenis: string, deskripsi: string): number => {
  ...
  if (J && J !== "lainnya") {
    ...
    if (J.includes("apd") || J.includes("tunjuk sebut") || J.includes("tunjuk-sebut") || J.includes("helm")) return 3;
    ...
    if (J.includes("selesai") || J.includes("pekerjaan selesai") || J.includes("sla") || J.includes("rpt") || J.includes("rct") || J.includes("rating") || J.includes("star")) return 11;
  }
  ...
  if (D) {
    ...
    if (D.includes("apd") || D.includes("tunjuk sebut") || D.includes("tunjuk-sebut") || D.includes("helm")) return 3;
    ...
    if (D.includes("selesai") || D.includes("pekerjaan selesai") || D.includes("sla") || D.includes("rpt") || D.includes("rct") || D.includes("rating") || D.includes("star")) return 11;
  }

  return 0; // Default to CCTV (Category 0) if completely unclassified so count is preserved
};
...
export const AnomaliPage: React.FC<AnomaliPageProps> = ({ data }) => {
  // Dynamic ULP list with default fallback for UP3 Padang
  const ulpList = useMemo(() => {
    if (data?.allUlps && Array.isArray(data.allUlps) && data.allUlps.length > 0) {
      return data.allUlps.map(u => {
        const cleanName = u.toUpperCase().replace(/^ULP\s+/i, '').trim();
        return {
    ...
    return [
      ...
      { id: "pahaman", name: "PAHAMAN" },
      { id: "belanti", name: "BELANTI" },
      { id: "lubuk_alung", name: "LUBUK ALUNG" },
      { id: "pariaman", name: "PARIAMAN" },
      { id: "indarung", name: "INDARUNG" }
    ];
  }, [data?.allUlps]);
  ...
  // Effective Anomali List (either passed directly or derived from live data)
  const effectiveAnomaliList = useMemo(() => {
    if (data?.anomaliList && Array.isArray(data.anomaliList) && data.anomaliList.length > 0) {
      return data.anomaliList;
    }

    const list: any[][] = [];

    // Synthesize WO Anomalies
    if (data?.rawWoRows && data?.woIndices) {
      const { woIndices, rawWoRows } = data;
      rawWoRows.forEach(row => {
        const apktNo = woIndices.apktNo !== undefined && woIndices.apktNo !== -1 && row[woIndices.apktNo] 
          ? String(row[woIndices.apktNo]) 
          : (row[13] || row[0] || "-");
        
        const officer = woIndices.name !== -1 && row[woIndices.name] ? String(row[woIndices.name]).trim() : "Tidak Terisi";
        const ulp = woIndices.ulp !== -1 && row[woIndices.ulp] ? String(row[woIndices.ulp]).trim() : "Tidak Terisi";
        const cctvVal = woIndices.cctv !== -1 && row[woIndices.cctv] ? String(row[woIndices.cctv]).toUpperCase() : "";
        const isCctv = cctvVal.includes("CCTV");

        let rpt = -1;
        if (woIndices.rpt !== -1 && row[woIndices.rpt]) {
          const parsed = parseFloat(String(row[woIndices.rpt]).replace(",", "."));
          if (!isNaN(parsed)) rpt = parsed;
        }

        let rct = -1;
        if (woIndices.rct !== -1 && row[woIndices.rct]) {
          const parsed = parseFloat(String(row[woIndices.rct]).replace(",", "."));
          if (!isNaN(parsed)) rct = parsed;
        }

        const tglLapor = woIndices.tglLapor !== -1 && row[woIndices.tglLapor] 
          ? String(row[woIndices.tglLapor]) 
          : (row[2] || "-");

        if (!isCctv) {
          list.push([apktNo, tglLapor, officer, ulp, "CCTV", "WO Tanpa CCTV", rpt >= 0 ? rpt : "-", rct >= 0 ? rct : "-"]);
        }
        if (rpt >= 30 || rct >= 180) {
          list.push([apktNo, tglLapor, officer, ulp, "Laporan Pekerjaan Selesai", `WO Over SLA (RPT: ${rpt}m, RCT: ${rct}m)`, rpt >= 0 ? rpt : "-", rct >= 0 ? rct : "-"]);
        }
      });
    }

    // Synthesize PO Anomalies
    if (data?.rawPoRows && data?.poIndices) {
      const { poIndices, rawPoRows } = data;
      rawPoRows.forEach(row => {
        const taskId = poIndices.id !== undefined && poIndices.id !== -1 && row[poIndices.id] 
          ? String(row[poIndices.id]) 
          : (row[4] || row[0] || "-");

        const officer = poIndices.name !== -1 && row[poIndices.name] ? String(row[poIndices.name]).trim() : "Tidak Terisi";
        const ulp = poIndices.ulp !== -1 && row[poIndices.ulp] ? String(row[poIndices.ulp]).trim() : "Tidak Terisi";
        const cctvVal = poIndices.cctv !== -1 && row[poIndices.cctv] ? String(row[poIndices.cctv]).toUpperCase() : "";
        const isCctv = cctvVal.includes("CCTV");

        const tgl = row[1] || row[2] || "-";

        if (!isCctv) {
          list.push([taskId, tgl, officer, ulp, "CCTV", "PO Tanpa CCTV", "-", "-"]);
        }
      });
    }

    // Synthesize Low Rating Anomalies
    if (data?.rating?.rating12List) {
      data.rating.rating12List.forEach(row => {
        list.push([
          String(row[0] || row[1] || "-"),
          String(row[1] || "-"),
          String(row[2] || "Tidak Terisi"),
          String(row[3] || "Tidak Terisi"),
  ...
  // Calculate ULP Pivot Table first so total counts can be derived from it
  const ulpPivotData = useMemo(() => {
    if (!effectiveAnomaliList) return [];
    ...
    effectiveAnomaliList.forEach(row => {
      ...
      const matchedUlp = ulpList.find(u => {
        ...
        return rawUlp === target || rawUlp.includes(target) || target.includes(rawUlp);
      });
      const targetKey = matchedUlp ? matchedUlp.name : rawUlp;
      ...
      if (hasStatusColumns) {
        ...
        if (parts.length > 0) {
          parts.forEach(part => {
            ...
            if (catIdx !== -1) {
              groups[targetKey].counts[catIdx]++;
            }
          });
        } else {
    ...
    // Compute exact totals per group from category counts
    Object.values(groups).forEach(g => {
      g.total = g.counts.reduce((a, b) => a + b, 0);
    });

    return Object.values(groups).sort((a, b) => b.total - a.total);
  }, [effectiveAnomaliList, ulpList]);

  // Pre-calculate ULP counts map directly from ulpPivotData
  const ulpCountsMap = useMemo(() => {
    ...
    ulpPivotData.forEach(g => {
      countsMap[g.ulp.toUpperCase()] = g.total;
    });

    return countsMap;
  }, [ulpPivotData, ulpList]);

  // UP3 total counts derived directly as the accumulation of all ULP findings
  const totalUp3Anomali = useMemo(() => {
    const sumUlpCards = ulpList.reduce((acc, u) => acc + (ulpCountsMap[u.name.toUpperCase()] || 0), 0);
    const sumPivot = ulpPivotData.reduce((acc, u) => acc + u.total, 0);
    return Math.max(sumUlpCards, sumPivot);
  }, [ulpList, ulpCountsMap, ulpPivotData]);
  ...
  // Debounce input to keep typing fluid
  useEffect(() => {
  ...
  const getAnomaliRowsForUnit = (unitName: string | null): any[][] => {
    if (!effectiveAnomaliList || !unitName) return [];
    
    if (unitName.toUpperCase().includes("UP3")) {
      return effectiveAnomaliList;
    }
    
    const target = unitName.toUpperCase().trim();
    return effectiveAnomaliList.filter(row => {
  ...
  const pivotData: PivotRow[] = useMemo(() => {
    ...
    // Compute exact totals per officer group from category counts
    Object.values(groups).forEach(g => {
      g.total = g.counts.reduce((a, b) => a + b, 0);
    });

    return Object.values(groups).filter(g => g.total > 0).sort((a, b) => b.total - a.total);
  }, [effectiveAnomaliList]);

  // Filter pivot table rows
  ...
  // Filtered Data for TINDAK LANJUT ANOMALI Table
  ...
  }, [effectiveAnomaliList, pivotSearch, pivotUlpSelect]);
  ...
  // Chart data for KETERANGAN ANOMALI
  ...
  // Chart data for ANOMALI PER ULP
  ...
  const openModalForCell = (petugas: string, ulp: string, categoryIdx: number | null, categoryLabel?: string) => {
    ...
    const filtered = (effectiveAnomaliList || []).filter(row => {
  ...
  return (
    <div id="anomali_page_outer" className="flex flex-col gap-6 p-6 bg-slate-50/50 rounded-2xl min-h-full">
      ...
      {/* Section 3: Tabel Per ULP as requested with exactly matched headers */}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 mb-6" id="three_features_row">
        ...
        <div className="lg:col-span-5 flex flex-col gap-6" id="left_stacked_charts">
          ...
          {/* Bottom Chart: ANOMALI PER ULP (Bar chart) */}
                            ...
                            {uploadedEvidens[rowId]?.fotoEviden1 ? (
                              <div className="flex flex-col items-center gap-1">
                                <div 
                                  ...
                                  className="w-10 h-10 rounded overflow-hidden border border-cyan-400 group relative shadow-sm hover:scale-[1.05] transition-transform cursor-pointer mx-auto"
            ...
            >
              ...
              <div className="bg-slate-50 p-4 border-t border-gray-100 flex flex-col sm:flex-row items-center justify-between gap-3">
                <span className="text-[10px] text-gray-400 font-black tracking-widest uppercase">
                  PLN ELECTRICITY DATA SERVICE • ANOMALI SYSTEM
                </span>
